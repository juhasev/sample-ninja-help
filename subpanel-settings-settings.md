...
#### Support URL
You should use the support URL when you prefer to send the panelists a support request form rather than have them email your support team. You may use the following variables to pass in extra information.
...
However, if you ask your panelists to whitelist your sending email address, they must do so using the actual email address.
...
Deliver survey notifications to panelists' web browsers in real-time. The user doesn't have to be on the website if their web browser is open. The push notifications dialog is automatically displayed on all termination pages (completed, profile, quota), the member app, and the email confirmation page. Push notifications work in all desktop browsers, including Chrome, Edge, and Firefox, as long as the user accepts them when prompted. Sample Ninja will automatically ask to enable notifications once a month, even if the panelist initially declined.

#### Enable mobile app
This option is only available if your panel has a mobile app enabled. Enabling this setting displays the mobile app download banner to users on landing pages and enables your panelists to log in using the mobile app.
